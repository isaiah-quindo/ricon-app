...
@section('title', 'New Race Category')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-6">Category Details</h2>

        <form method="POST" action="{{ route('admin.race-categories.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" id="max_slots" name="max_slots"
                           value="{{ old('max_slots') }}" required min="1"
                           placeholder="e.g. 300"
                ...
                <div>
                    ...
                    <input type="number" id="bib_start_number" name="bib_start_number"
                           value="{{ old('bib_start_number') }}" required min="1"
                    ...
                    <p class="text-xs text-gray-400 mt-1">First bib number assigned in this category.</p>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              placeholder="Optional description shown on the registration form..."
                              class="w-full rounded-lg border {{ $errors->has('description') ? 'border-red-400' : 'border-gray-200' }} text-sm px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
